...
#else
            active = Keyboard.current?.FindKeyOnCurrentKeyboardLayout(_inputButtonName)?.isPressed == true;
#endif
        }
        //Push To Talk
...
#else
            if (Keyboard.current?.FindKeyOnCurrentKeyboardLayout(_inputButtonName)?.wasPressedThisFrame == true)
#endif
            {
                active = !active;
            }
